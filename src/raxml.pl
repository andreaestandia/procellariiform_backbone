...
# November 2018, Andrea Estandia
# Perl script to run raxml + bootstrap for each locus.
# this is *not* part of the TICR pipeline
# adapted from Solis-Lemus, Yang and Ane (2016) scripts & https://github.com/crsl4/InconsistencySpeciesTreeGeneFlow/blob/master/scripts/estGeneTrees/raxml.pl:
...
my $convertphylip = 1;
my $doastral = 1;
my $raxml = 'raxmlHPC-PTHREADS-SSE3'; # executable
my $astral = '/ddn/apps/Cluster-Apps/dbl/astral/ASTRAL-master/astral.5.6.1.jar'; # adapt to your system
...
for my $ig (0 .. $#genes){
    ...
    my $str = "$raxml -T 40 -f a -m GTRGAMMA -N $numboot";
...
#my $astralcmd = "java -jar $astral -i $raxmlOUT --gene-only -o $astralOUT > $astralLOG 2>&1";
#open FHlog, ">> $logfile";
#if ($doastral){
